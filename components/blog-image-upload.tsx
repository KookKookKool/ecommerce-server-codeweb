...
const ImageUpload = ({
  ...
  const onUpload = async (e: any) => {
    ...
    const uploadTask = uploadBytesResumable(
      ref(storage, `blog/${Date.now()}-${file.name}`),
      file,
    ...
    <>
      {value && value.length > 0 ? (
        <div className="mb-4 flex items-center gap-4">
          {value.map(url => (
            <div className="relative w-full h-[50vh] aspect-w-16 aspect-h-9 rounded-md overflow-hidden" key={url}>
              <Image
                fill
        ...
        <div className="w-128 h-72 rounded-md overflow-hidden border border-dashed border-gray-200 flex items-center justify-center flex-col gap-3">
          {isLoading ? (
            <>
